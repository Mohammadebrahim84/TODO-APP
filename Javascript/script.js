...
function main() {
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    allTodos.forEach(todo => {
      const todoText = todo.querySelector(".item").textContent.toLowerCase();
...
function updateTodoInStorage(index, updatedTodo) {
  const todos = JSON.parse(localStorage.getItem("todos"));
...
function makeTodoElement(todoArray) {
  ...
  todoArray.forEach((todoObject, index) => {
    ...
    deadlineInput.value = todoObject.deadline;
    ...
    editBtn.addEventListener('click', () => {
      const newText = prompt("Edit your todo:", todoObject.item);
      if (newText) {
        todoObject.item = newText;
        item.textContent = newText;
        updateTodoInStorage(index, todoObject);
      }
    });

    deadlineInput.addEventListener('change', (e) => {
      todoObject.deadline = e.target.value;
      updateTodoInStorage(index, todoObject);
    });
